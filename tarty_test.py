...
# Import classes and functions from tarty.py
from tarty import (
    DEFAULT_SSH_PORT,
    DEFAULT_UPDATE_HOUR,
...
def test_run_tart_command_success(mock_run):
    ...
    result = CommandRunner.run_tart_command(['list'])
...
def test_run_ssh_command_success(mock_run):
    ...
    result = CommandRunner.run_ssh_command('192.168.1.100', 'admin', 'ls')
    expected_cmd = [
        'ssh',
        '-o',
...
# --- SSHClient Tests ---
@patch('tarty.CommandRunner.run_ssh_command')
...
@patch('socket.socket')
def test_ssh_client_is_available_success(mock_socket):
    mock_sock_instance = MagicMock()
    mock_socket.return_value.__enter__.return_value = mock_sock_instance
    mock_sock_instance.connect_ex.return_value = (
        0  # Simulate successful connection
    )

    client = SSHClient('192.168.1.100', 'admin')
    assert client.is_available(timeout=1) is True
    mock_sock_instance.connect_ex.assert_called_once_with(
        ('192.168.1.100', DEFAULT_SSH_PORT)
    )


@patch('socket.socket')
def test_ssh_client_is_available_timeout(mock_socket):
    mock_sock_instance = MagicMock()
    mock_socket.return_value.__enter__.return_value = mock_sock_instance
    mock_sock_instance.connect_ex.return_value = (
    ...
    client = SSHClient('192.168.1.100', 'admin')
    assert client.is_available(timeout=0.5, retry=0.1) is False
    # Check that connect_ex was called multiple times until timeout
    assert mock_sock_instance.connect_ex.call_count > 1


# --- TartVM Tests ---
@patch('tarty.SSHClient')
@patch('tarty.CommandRunner.run_tart_command')
...
def test_tart_vm_start_success(
    ...
    mock_thread.assert_called_once()  # Check that VM process was attempted to be started
...
def test_tart_vm_start_fail_get_ip(
    ...
    # Simulate tart run success, then tart ip failure (raises CalledProcessError)
    mock_run_tart_command.side_effect = [
        MagicMock(returncode=0),  # For tart run (thread target)
    ...
    mock_ssh_client_instance = MagicMock()
    mock_ssh_client_cls.return_value = mock_ssh_client_instance
    mock_ssh_client_instance.is_available.return_value = (
        False  # SSH is not available
    )
    ...
    )  # Should revert to stopped or stay stopped
    mock_thread.assert_called_once()
    # Ensure stop was called implicitly
...
def test_tart_vm_destroy_success(mock_run_tart_command):
    ...
    vm.state = (
        VMState.STOPPED
    )  # Assume stopped for simplicity, destroy handles stopping
...
def test_tart_vm_destroy_stops_first(mock_run_tart_command):
    vm = TartVM('test-vm', 'test-image')
    vm.state = VMState.RUNNING  # VM is running
    assert vm.destroy() is True
    # Check that stop was called, then delete
    mock_run_tart_command.assert_any_call(
        ['stop', 'test-vm'], check=True, capture_output=True
    )
    mock_run_tart_command.assert_any_call(
...
def test_register_runner_success(
    ...
    mock_vm = MagicMock()
    mock_vm.name = 'test-vm'  # Explicitly set name as string
...
def test_register_runner_config_fail(
    ...
    mock_ssh_client_instance = MagicMock()
    mock_ssh_client_cls.return_value = mock_ssh_client_instance
    mock_ssh_client_instance.execute_command.return_value = (
        False  # Configure fails
    )
...
@patch(
    'tarty.CommandRunner.run_tart_command',
...
def test_should_update_nightly_update(
    ...
    mock_datetime.now.return_value = datetime(
        2023, 1, 2, 2, 0, 0
    )  # Assuming DEFAULT_UPDATE_HOUR is 2
    mock_load_last_update.return_value = datetime(
        2023, 1, 1, 15, 0, 0
    )  # Yesterday, different hour
    manager = ImageManager(mock_image_config)
    assert manager.should_update() is True
...
@patch(
    'tarty.time.time', return_value=1234567890.0
)  # Mock time.time for predictable VM name
def test_update_runner_image_success(
    ...
    mock_image_config.convert_command = (
        'some_command'  # Ensure convert command is present
    )

    # Predict the temporary VM name based on mocked time.time()
    temp_vm_name_expected = f'tarty-convert-{int(mock_time.return_value)}'

    mock_temp_vm = MagicMock()
    mock_temp_vm.name = temp_vm_name_expected  # Explicitly set name as string
    mock_temp_vm.stop.return_value = True
    mock_create_temp_vm.return_value = mock_temp_vm

    manager = ImageManager(mock_image_config)
    manager.last_update = datetime(2023, 1, 1)  # Set initial last_update

    assert manager.update_runner_image() is True

    mock_create_temp_vm.assert_called_once_with(
        temp_vm_name_expected
    )  # Assert with expected name
    ...
    mock_run_tart_command.assert_called_once_with(
        [
            'rename',
            temp_vm_name_expected,
            mock_image_config.runner_image,
        ],  # Assert with expected name
        check=True,
        capture_output=True,
    )
...
@patch('tarty.ImageManager._delete_old_runner_image')
@patch(
    'tarty.ImageManager._run_conversion', return_value=False
)  # Conversion fails
@patch('tarty.ImageManager._create_temp_vm')
@patch('tarty.CommandRunner.run_tart_command')
...
def test_update_runner_image_conversion_fails(
    ...
    mock_run_conversion.assert_called_once_with(mock_temp_vm)
    mock_cleanup_temp_vm.assert_called_once_with(
        temp_vm_name_expected
    )  # Assert with expected name
    mock_run_tart_command.assert_not_called()  # Rename should not be called


# --- TartyOrchestrator Tests ---
@patch('tarty.TartyConfig')
@patch('tarty.GitHubRunnerManager')
@patch('tarty.ImageManager')
def test_orchestrator_init_success(
    mock_image_manager_cls, mock_runner_manager_cls, mock_tarty_config_cls
):
    mock_tarty_config_instance = MagicMock()
    mock_tarty_config_cls.return_value = mock_tarty_config_instance

    orchestrator = TartyOrchestrator('dummy_config.json')
    assert orchestrator.config == mock_tarty_config_instance
    assert isinstance(
        orchestrator.runner_manager, MagicMock
    )  # Check it's an instance of the mocked class
    assert isinstance(orchestrator.image_manager, MagicMock)
    assert orchestrator.running is False
    assert orchestrator.vms == {}


@patch('tarty.TartyConfig', side_effect=ConfigError('Test config error'))
@patch('sys.exit')
def test_orchestrator_init_config_error(mock_sys_exit, mock_tarty_config_cls):
    TartyOrchestrator('dummy_config.json')
...
def test_orchestrator_start_stop_loop(mock_sleep, mock_cycle):
    # Mock TartyConfig, GitHubRunnerManager, ImageManager for orchestrator init
    with (
        patch('tarty.TartyConfig'),
        patch('tarty.GitHubRunnerManager'),
        patch('tarty.ImageManager'),
        ...
        # Simulate running for 2 cycles then stopping
        mock_cycle.side_effect = [None, None, KeyboardInterrupt]

        orchestrator.start()

        assert mock_cycle.call_count == 3  # Called until KeyboardInterrupt
        assert orchestrator.running is False
...
def test_cleanup_completed_vms(mock_cleanup_vm):
    with (
        ...
        # Create some mock VMs, one running, one stopped
        vm1 = MagicMock()
        vm1.name = 'vm1'
        vm1.running = True
        vm2 = MagicMock()
...
def test_start_new_vms_if_needed(mock_time, mock_create_and_start_vm):
    with (
        ...
        orchestrator = TartyOrchestrator('dummy_config.json')
        orchestrator.vms = {}  # Start with no VMs

        # MAX_VMS is 2, so it should try to create up to 2
        orchestrator._start_new_vms_if_needed()

        # It should try to create one, succeed, and then break (as per the original code's `break` after success)
...
# Removed patches for class methods, will mock instance methods
@patch(
    'tarty.time.time', return_value=1234567890.0
)  # Mock time.time for predictable VM name
def test_create_and_start_vm_success(mock_time):
    with (
        ...
        mock_vm_instance.name = (
            'test-vm-instance'  # Explicitly set name as string
        )
        mock_vm_instance.start.return_value = True
        # Patch TartVM class directly for instantiation
        with patch(
...
def test_create_and_start_vm_image_creation_fail(mock_time):
    with (
        ...
        # Set return value on the instance mock
        ...
        # Ensure no further calls if image creation fails
...
def test_create_and_start_vm_runner_registration_fail(mock_time):
    with (
        ...
        with patch(
            ...
            mock_vm_instance.destroy.assert_called_once()  # Should destroy VM if registration fails
            assert vm_name not in orchestrator.vms


# Removed patch for TartVM.destroy, will assert on instance mock
def test_cleanup_vm():
    with (
        patch('tarty.TartyConfig'),
        patch('tarty.GitHubRunnerManager'),
        ...
        mock_vm = MagicMock()
        mock_vm.name = 'test-vm'  # Explicitly set name as string
        mock_vm.destroy.return_value = True  # Mock the instance method
...
@patch('tarty.datetime')  # Patch datetime for now()
def test_check_nightly_updates_performs_update(
    mock_datetime,
    mock_make_room,
):
    mock_now = datetime(2023, 1, 2, 10, 0, 0)  # Define mock_now
    mock_datetime.now.return_value = (
        mock_now  # Set return value for datetime.now()
    )

    with (
        patch('tarty.TartyConfig') as mock_tarty_config_cls,
        patch('tarty.GitHubRunnerManager'),
        patch('tarty.ImageManager') as mock_image_manager_cls,
    ):
        mock_config_instance = MagicMock(update_hour=10)
        mock_tarty_config_cls.return_value = mock_config_instance
        ...
        mock_image_manager_instance._save_last_update.return_value = (
            None  # Mock this to avoid file ops
        )

        orchestrator = TartyOrchestrator('dummy_config.json')
        orchestrator.last_update_check = None  # Ensure it checks

        orchestrator._check_nightly_updates()

        mock_image_manager_instance.should_update.assert_called_once_with(
            mock_config_instance.update_hour
        )
...
def test_check_nightly_updates_skips_update(
    ...
    with (
        ...
        mock_image_manager_instance = mock_image_manager_cls.return_value
        mock_image_manager_instance.should_update.return_value = (
            False  # Should not update
        )
        mock_image_manager_instance.update_runner_image.return_value = (
            True  # Set a default, though it shouldn't be called
        )
...
def test_make_room_for_update_kills_one_vm(mock_cleanup_vm):
    with (
        ...
        # It should kill exactly one VM
        mock_cleanup_vm.assert_called_once()
        # The killed VM should be one of the ones in the dict
